...
class GenAlArchNNv3(object):

    def __init__(self,n_pop=10,n_gen=17,n_hid = 5,
    ...
    def fit(self,data,target):
        ...
        for n in range(self.n_pop):
            ...
            w0 = 2*np.random.random((n_input,n_hid))-1 
            w1 = 2*np.random.random((n_hid,n_out))-1
            #Biases weights
            bw1 = 2*np.random.random((n_hid))-1
            bw2 = 2*np.random.random((n_out))-1
...
#                 print(self.pop.shape)
    ...
    def growing(gene):
        ...
        w0,w1,bw0,bw1 = gene
        w0 = np.hstack([w0,np.random.random(w0.shape[0]).reshape(w0.shape[0],1)])
        w1 = np.vstack([np.random.random(w1.shape[1]).reshape(1,w1.shape[1]),w1])
...
# ga = GenAlWeightsNN(n_pop=1000,max_gen=100,desired_fit=0.95)
